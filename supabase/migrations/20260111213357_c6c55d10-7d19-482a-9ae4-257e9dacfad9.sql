-- Update the function to filter notifications by user preferences
...
BEGIN
  ...
  IF NEW.is_present = true AND NEW.visible_to_others = true THEN
    ...
    LOOP
      ...
      -- Get exhausted user's profile for city preference
      SELECT city INTO exhausted_user_profile
      FROM profiles
