-- Update handle_new_connection_request with new rotated secret
...
BEGIN
  -- Get the current internal secret
  ...
  -- Send push notification with dynamic internal secret
  PERFORM net.http_post(
    url := 'https://kcmchxcbtkevvxpwatec.supabase.co/functions/v1/send-push-notification',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'x-internal-secret', COALESCE(v_internal_secret, 'kiki-internal-cba2a2cf123082effea1e201c8896cb3')
    ),
...
-- Update handle_connection_accepted with new rotated secret
...
-- Update handle_premium_ghost_message with new rotated secret
...
-- Update handle_profile_visit_notification with new rotated secret
