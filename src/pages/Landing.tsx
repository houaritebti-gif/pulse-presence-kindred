...
const Landing = () => {
  ...
  const quotes = ["Las mejores conexiones no se fuerzan.", "Aquí no hay matches, hay momentos.", "Menos ruido, más chispa."];
  return <div ref={containerRef} className="min-h-screen bg-background overflow-x-hidden">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden">
        ...
        y: backgroundY
          ...
          <div className="absolute bottom-1/3 right-1/4 w-48 h-48 bg-accent/10 rounded-full blur-2xl animate-float" style={{
          animationDelay: "2s"
        }} />
          <div className="absolute top-2/3 left-1/3 w-24 h-24 bg-primary/15 rounded-full blur-xl animate-float" style={{
          animationDelay: "4s"
        }} />
        ...
        <motion.div className="relative z-10 max-w-2xl w-full text-center space-y-8" style={{
        opacity
      }}>
          ...
          {/* Feature highlights */}
          <motion.div className="flex flex-wrap justify-center gap-4 pt-4" initial={{
          opacity: 0,
          y: 30
        }} animate={{
          opacity: 1,
          ...
          ease: "easeOut"
            ...
            {["Sin algoritmos", "Sin presión", "Sin ruido"].map((tag, index) => <div key={index} className="px-4 py-2 rounded-full bg-card/80 backdrop-blur-sm border border-border/20">
                <span className="font-body text-sm text-card-foreground/80">{tag}</span>
              </div>)}
          </motion.div>

          {/* Subtle description */}
          <motion.p className="font-body text-base md:text-lg text-muted-foreground max-w-md mx-auto leading-relaxed" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} transition={{
          ...
          delay: 1
            ...
            <Button variant="kiki" size="xl" onClick={() => navigate("/auth")} className="w-full max-w-xs relative group overflow-hidden">
              <span className="relative z-10 flex items-center justify-center gap-2">
                <Heart className="w-5 h-5 fill-current" />
                Entrar
              </span>
              <div className="absolute inset-0 bg-gradient-to-r from-primary via-accent to-primary bg-[length:200%_100%] opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{
          ...
          delay: 1.5
            ...
            <motion.div className="w-6 h-10 border-2 border-muted-foreground/30 rounded-full mx-auto flex justify-center" animate={{
            y: [0, 8, 0]
          }} transition={{
            duration: 1.5,
            repeat: Infinity,
      ...
      <section className="min-h-screen flex items-center py-24 px-6 relative">
        ...
        <div className="max-w-6xl mx-auto w-full relative z-10">
          ...
          <motion.div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6" initial="hidden" whileInView="visible" viewport={{
          once: true,
          margin: "-50px"
        }} variants={staggerContainer}>
            {features.map((feature, index) => <motion.div key={index} variants={scaleIn} className="group p-8 rounded-3xl backdrop-blur-sm border border-border/20 hover:border-primary/30 transition-all duration-500 bg-destructive-foreground">
      ...
      <section className="min-h-screen flex items-center py-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto w-full relative z-10">
          ...
          <motion.div className="space-y-12" initial="hidden" whileInView="visible" viewport={{
          once: true,
          margin: "-50px"
        }} variants={staggerContainer}>
            {howItWorks.map((item, index) => <motion.div key={index} variants={fadeInUp} className="flex gap-8 items-start group">
      ...
      <section className="min-h-screen flex items-center py-24 px-6 relative">
        ...
        <div className="max-w-4xl mx-auto w-full relative z-10">
          <motion.div className="space-y-16" initial="hidden" whileInView="visible" viewport={{
          once: true,
          margin: "-100px"
        }} variants={staggerContainer}>
            {quotes.map((quote, index) => <motion.div key={index} variants={fadeInUp} className="text-center">
      ...
      <section className="min-h-[70vh] flex items-center justify-center py-24 px-6 relative overflow-hidden">
        ...
        <motion.div className="relative z-10 text-center space-y-8 max-w-xl" initial="hidden" whileInView="visible" viewport={{
        once: true,
        margin: "-100px"
      }} variants={staggerContainer}>
          ...
          <motion.h2 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-foreground" variants={fadeInUp}>
            ¿Tienes chispa?
          </motion.h2>
          
          <motion.p className="font-body text-lg text-muted-foreground" variants={fadeInUp}>
            Solo para quienes entienden.
          </motion.p>
          
          <motion.div variants={fadeInUp}>
            <Button variant="kiki" size="xl" onClick={() => navigate("/auth")} className="relative group overflow-hidden">
              <span className="relative z-10 flex items-center justify-center gap-2">
